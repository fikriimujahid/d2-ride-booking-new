...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // JWT lifecycle:
    // - Cognito issues access+id tokens (short lived) and refresh token (longer lived).
    // - Cognito Identity JS stores its session in configured Storage (here: sessionStorage).
    // - On page load we restore the session and continue.
    cognito
      .getCurrentSession()
      .then((s) => {
